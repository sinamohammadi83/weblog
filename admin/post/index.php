...
if (isset($_GET['search'])){
    ...
    $query = "SELECT * From posts WHERE status = 'publish' and title like '%$search%' or description like '%$search%';";
...
<div class="bg-white shadow w-full lg:max-w-screen-xl rounded-md p-4 h-5/6 overflow-hidden">
        <form method="get">
            <div class="bg-sky-200 rounded-md w-full p-1 mb-5">
                ...
                <input type="hidden" name="s" value="post">
            ...
            foreach ($posts as $post){
                ...
                <div class="flex justify-between pt-4 pb-1 ">
                    <div class="flex pr-3 text-sm">
                        ...
                        <div class="w-20"><?php if ($post['user_id'] == 0){ echo "admin"; } else {$user = $pdoObj->query("SELECT * FROM users WHERE id='$post[user_id]'")->fetch();echo $user['firstname']." ".$user["lastname"];}  ?></div>
                    </div>
