...
if (isset($_GET['search'])){
    ...
    $categories = $pdoObj->query($query)->fetchAll();
...
<div class="flex justify-between w-full lg:max-w-screen-xl items-center mb-10">
    <div class="text-xl font-bold">لیست دسته بندی</div>
    <a href="index.php?s=category&a=create" class="bg-white rounded-md p-2 shadow">
...
<div class="bg-white shadow w-full lg:max-w-screen-xl rounded-md p-4 h-5/6 overflow-hidden">
        <form method="get">
            <div class="bg-sky-200 rounded-md w-full p-1 mb-5">
                <input type="text" placeholder="عنوان ..." class="p-2 rounded-md text-sm outline-none w-72" name="search">
                <input type="hidden" name="s" value="category">
    ...
    <div class="divide-y divide-gray-300 h-full overflow-y-auto pb-24">
        ...
        if ($categories){
            ...
            <div class="flex justify-between py-4">
                <div class="flex pr-3 text-sm">
                    <div class="w-10"><?php echo $category['id'] ?></div>
                    <div class="w-20 truncate"><?php echo $category['title'] ?></div>
                </div>
                <div class="w-44 justify-center text-sm flex gap-x-2">
                    <a href="index.php?s=category&a=create&id=<?php echo $category['id'] ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                        </svg>
                    </a>
                    <a href="index.php?s=category&a=delete&id=<?php echo $category['id'] ?>" class="mr-2 block">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
